...
export const Connexion = () => {
  ...
  return user ? (
    <button onClick={logout} className="h-12 w-12">
      <img src={user.photoURL || ""} alt="user" className="w-full h-full leading-loose rounded-full" />
    </button>
  ) : (
    <button onClick={login} className="h-12 w-12">
      <svg

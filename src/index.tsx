...
import koala from "./koala-silhouette.png";
...
const App: React.FunctionComponent = () => {
  ...
  useEffect(() => {
    ...
    animalsDatabase.onSnapshot(snapshot => {
      console.log("YEAAAA");
      setAnimals(
        // @ts-ignore
        snapshot.docs.map(d => {
  ...
  return (
    <UserContext.Provider value={user}>
      <AnimalContext.Provider value={animals}>
        <Router>
          <Header className="h-24 bg-yellow-700">
            <Link to="/">
              <img src={koala} className="h-20 justify-center items-center" alt="koala header" />
            </Link>
            <div className="justify-center items-center px-8 ml-auto flex">
              {user ? (
                <Link to="/add">
                  <svg viewBox="0 0 478.703 478.703" className="h-12 w-12 mr-2">
                    <g>
